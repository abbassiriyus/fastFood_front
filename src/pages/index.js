...
import GlCarulsel from '../components/GlCarusel'
...
const FoodCard = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`${url}/users`).then(res => {
      const data = res.data
        ...
        .map(item => ({
          ...item,
          _sortOrder: item.orders + Math.random() * 0.01
        }))
  ...
  useEffect(() => {
    ...
    if (!stol) {
      window.location.href = "https://4a.uz";
    } else {
        ...
        <div style={{ maxWidth: '600px', margin: 'auto' }}>
          ...
          <div className={styles.cardContainer}>
            <div className={styles.logoContainer}>
              {foodLogos.map(logo => (
                <div
                  onClick={() => { window.location = `/food/?fastfood=${logo.id}` }}
